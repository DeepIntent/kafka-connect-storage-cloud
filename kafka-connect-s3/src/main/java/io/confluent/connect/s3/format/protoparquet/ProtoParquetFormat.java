...
public class ProtoParquetFormat implements Format<S3SinkConnectorConfig, String> {
  private final S3Storage storage;

  // DO NOT change this signature, it is required for instantiation via reflection
  public ProtoParquetFormat(S3Storage storage) {
    this.storage = storage;
  }

  @Override
  public RecordWriterProvider<S3SinkConnectorConfig> getRecordWriterProvider() {
    return new ProtoParquetRecordWriterProvider(
            storage, storage.conf().getProtobufMessageClass());
  }
